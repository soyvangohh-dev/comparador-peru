...
// Resto del código (state, funciones, etc.)
const state = {
  period: '360',
  filter: 'all',
  amount: 5000,
...
function startCountdown() {
  ...
  state.countdown = 30;
  if ($('countdownNum')) $('countdownNum').textContent = '30';
  if ($('updateBar')) $('updateBar').style.width = '0%';

  state.countdownTimer = setInterval(() => {
    state.countdown--;
    if ($('countdownNum')) $('countdownNum').textContent = state.countdown;
    if ($('updateBar')) $('updateBar').style.width = ((30 - state.countdown) / 30 * 100) + '%';
...
function renderStats(rows) {
  if ($('statsInstitutions')) $('statsInstitutions').textContent = rows.length;
  if ($('statsMaxRate')) $('statsMaxRate').textContent = (rows.length ? Math.max(...rows.map(r=>r.rate)) : 0).toFixed(2) + '%';
  if ($('statsMaxEarning')) $('statsMaxEarning').textContent = rows.length ? fmt.money(Math.max(...rows.map(r=>r.earning))) : '—';
  if ($('statsMinAmount')) $('statsMinAmount').textContent = rows.length ? fmt.money(Math.min(...rows.map(r=>r.min_amount))) : '—';
}
...
function renderBestCards() {
  ...
  types.forEach(type => {
    ...
    if (!best) return;
    html += `<div class="best-card" onclick="window.open('${best.url}','_blank','noopener')">
  ...
  if ($('bestCards')) $('bestCards').innerHTML = html;
}
...
function renderAll() {
  ...
  renderTable(rows);
  renderBestCards();
  if ($('lastUpdate')) $('lastUpdate').textContent = 'Act: ' + new Date().toLocaleTimeString('es-PE', { hour:'2-digit', minute:'2-digit', second:'2-digit' });
  if ($('loadingOverlay')) $('loadingOverlay').style.display = 'none';
  if ($('tableSection')) $('tableSection').style.display = 'block';
  if ($('refRate')) $('refRate').textContent = state.reference_rate.toFixed(2) + '%';
  if ($('periodLabel')) $('periodLabel').textContent = PERIOD_LABELS[state.period];
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (amountInput) amountInput.addEventListener('input', onAmountChange);
